...
function parseTodoWrite(raw: unknown) { try { const obj = typeof raw === 'string' ? JSON.parse(raw) : raw; const list = (obj as any)?.todos; if (Array.isArray(list)) { todos.value = list.map((t: any) => ({ content: t.content || '', status: t.status || 'pending' })); } } catch { /* */ } }
function parseUpdatePlan(raw: unknown) { try { const obj = typeof raw === 'string' ? JSON.parse(raw) : raw; const list = (obj as any)?.plan; if (Array.isArray(list)) { plan.value = { explanation: (obj as any)?.explanation, steps: list.map((s: any) => ({ step: s.step || '', status: s.status || 'pending' })) }; } } catch { /* */ } }
...
onMounted(() => {
    window.addEventListener('message', (e) => { const msg = e.data; switch (msg.type) {
        ...
        case 'tuiToolResult': { const o = msg.toolResult?.output ?? msg.toolResult?.error; chatRef.value?.updateToolResult(msg.toolResult?.callId || '', o, msg.toolResult?.status === 'error' ? 'error' : 'success'); const tn = msg.toolResult?.toolName || ''; if (tn === 'todo_write') parseTodoWrite(o); if (tn === 'update_plan') parseUpdatePlan(o); break; }
